<div>
    ...
    <article class=" w-full md:flex mx-auto">
        <section class="w-full md:w-1/2 mx-4">
            <x-image-gallery :images="$animal->images" class="w-full md:flex flex-col" />
        </section>
        <section class="w-full md:w-1/2">
            ...
            <article class="mx-4 my-4 shadow-md rounded-lg">
                <h3 class="font-bold">Message from my caretakers</h3>
                <p>{{$animal->message}}</p>
            </article>
            <a href="/shelters/{{$animal->shelter_id}}" class="flex justify-evenly mx-4 rounded-lg shadow-md my-2">
                <img src="{{ asset('storage/' . $animal->shelter->images->first()?->path) }}" alt="animal image"
                    class="size-48" />
                <div>
                    <h3 class="font-bold mb-4">{{$animal->shelter->name}}</h3>
